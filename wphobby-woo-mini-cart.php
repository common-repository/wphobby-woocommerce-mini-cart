...
if( ! function_exists( 'whmc_install_premium_admin_notice' ) ) {
   /**
    * Display an admin notice if woocommerce is deactivated
    *
    * @since 1.0.0
    ...
    * @use admin_notices hooks
    */
   function whmc_install_premium_admin_notice() {

      echo sprintf(
                 '<div class="notice alert success"><p>%s</p></div>',
                 sprintf(
                     __("Like WPHobby WooCommerce Mini Cart free version? You can also buy our premium version <a href='https://codecanyon.net/item/wphobby-woocommerce-mini-cart/24201931'>Here</a>", "wphobby-woo-mini-cart")
                  )
           );
   }
}
...
if( ! function_exists( 'whmc_install' ) ){
   function whmc_install() {
      ...
      if ( ! function_exists( 'WC' ) ) {
         ...
         add_action( 'admin_notices', 'whmc_install_premium_admin_notice' );
